...
class AzureContentUnderstandingClient:
    ...
    # CU service file size limits (bytes)
    # https://learn.microsoft.com/en-us/azure/ai-services/content-understanding/service-limits
    # "Text" types (.eml, .msg, .txt, .html, .md, .xml) have a 1MB limit.
    # "Document" types (.pdf, .docx, .xlsx, .pptx) and images have higher limits.
    FILE_SIZE_LIMIT_TEXT: int = 1 * 1024 * 1024       # 1 MB for text/email types
    FILE_SIZE_LIMIT_DOCUMENT: int = 20 * 1024 * 1024  # 20 MB for document types
    FILE_SIZE_LIMIT_IMAGE: int = 20 * 1024 * 1024     # 20 MB for image types

    # File extensions classified as "Text" by CU (subject to 1MB limit)
    TEXT_FILE_TYPES: List[str] = [
        ".eml", ".msg", ".txt", ".html", ".md", ".xml",
    ]

    # File extensions classified as "Document" by CU (subject to 20MB limit)
    DOCUMENT_FILE_TYPES: List[str] = [
        ".pdf", ".docx", ".xlsx", ".pptx",
    ]

    # File extensions classified as "Image" by CU (subject to 20MB limit)
    IMAGE_FILE_TYPES: List[str] = [
        ".tiff", ".tif", ".jpg", ".jpeg", ".png", ".bmp", ".heif",
    ]
    ...
    def get_file_type_category(file_ext: str) -> str:
        """
        Returns the CU file type category for a given extension.

        CU classifies files into categories with different size limits:
        - "text": .eml, .msg, .txt, .html, .md, .xml → 1 MB limit
        - "document": .pdf, .docx, .xlsx, .pptx → 20 MB limit
        - "image": .tiff, .tif, .jpg, .jpeg, .png, .bmp, .heif → 20 MB limit
        - "unknown": not in any known category

        Args:
            file_ext (str): The file extension (e.g., ".eml", ".pdf").

        Returns:
            str: The category name ("text", "document", "image", or "unknown").
        """
        ext = file_ext.lower()
        if ext in AzureContentUnderstandingClient.TEXT_FILE_TYPES:
            return "text"
        elif ext in AzureContentUnderstandingClient.DOCUMENT_FILE_TYPES:
            return "document"
        elif ext in AzureContentUnderstandingClient.IMAGE_FILE_TYPES:
            return "image"
        return "unknown"

    @staticmethod
    def get_file_size_limit(file_ext: str) -> Optional[int]:
        """
        Returns the CU service file size limit (in bytes) for the given extension.

        Args:
            file_ext (str): The file extension (e.g., ".eml", ".pdf").

        Returns:
            int or None: The size limit in bytes, or None if unknown.
        """
        category = AzureContentUnderstandingClient.get_file_type_category(file_ext)
        if category == "text":
            return AzureContentUnderstandingClient.FILE_SIZE_LIMIT_TEXT
        elif category == "document":
            return AzureContentUnderstandingClient.FILE_SIZE_LIMIT_DOCUMENT
        elif category == "image":
            return AzureContentUnderstandingClient.FILE_SIZE_LIMIT_IMAGE
        return None

    @staticmethod
    def check_file_size(file_path: Path) -> Optional[str]:
        """
        Checks whether a file exceeds the CU service size limit for its type.

        The CU service enforces different size limits per file type category:
        - Text/email types (.eml, .msg, .txt, .html, .md, .xml): 1 MB
        - Document types (.pdf, .docx, .xlsx, .pptx): 20 MB
        - Image types (.tiff, .jpg, .png, .bmp, .heif): 20 MB

        Both :analyzeBinary and :analyze (URL-based) endpoints enforce the
        same limits. There is no endpoint workaround for oversized files.

        Args:
            file_path (Path): Path to the file to check.

        Returns:
            str or None: Warning/error message if the file exceeds the limit,
                         or None if the file is within limits.
        """
        if not file_path.is_file():
            return None

        file_ext = file_path.suffix.lower()
        size_limit = AzureContentUnderstandingClient.get_file_size_limit(file_ext)
        ...
        if file_size > size_limit:
            ...
            return (
                f"File '{file_path.name}' ({file_mb:.1f} MB) exceeds the "
                f"{limit_mb:.0f} MB limit for '{category}' file types "
                f"(extension: {file_ext}). Both :analyzeBinary and :analyze "
                f"endpoints enforce this limit."
            )
        return None
    ...
    def validate_setup(self, verbose: bool = False) -> Dict[str, Any]:
        """
        Validate connectivity and authentication against the CU service.

        This performs a lightweight GET to the defaults endpoint and
        returns the defaults payload. Warnings are printed for missing
        model deployments, but those are not treated as fatal because
        layout-only workflows do not require them.

        Args:
            verbose (bool): If True, print validation progress and warnings.

        Returns:
            dict: Defaults payload returned by the service.
        ...
        if verbose:
            ...
            print(f"API Version: {self._api_version}")
        ...
        if verbose:
            ...
            if not model_deployments:
                print("⚠️  No model deployments configured. Custom analyzers will fail until defaults are set.")
            elif "gpt-4.1" not in model_deployments:
                print("⚠️  Missing default deployment for 'gpt-4.1' (required for custom analyzers).")
    ...
    def begin_analyze_binary(
        ...
        Performs a pre-flight file size check before sending to the API.
        CU enforces different size limits per file type category:
        ...
        Raises:
            ValueError: If the file location is not a valid file path or exceeds size limit.
            HTTPError: If the HTTP request returned an unsuccessful status code.
        """
        ...
        # Pre-flight size check to avoid sending oversized files to the API
        size_warning = self.check_file_size(file_path)
        if size_warning:
    ...
    def poll_result(
        ...
        while True:
            ...
            try:
                ...
                self._logger.info("Polling interrupted. Waiting for final result...")
                # Continue polling until we get a result or timeout
                time.sleep(1)
